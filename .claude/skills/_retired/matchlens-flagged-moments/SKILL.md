---
name: matchlens-flagged-moments
description: Create a branded Match Lens flagged-moments report as a PDF: timestamped key moments with a match timeline and a moments rail. Trigger when the user asks for flagged moments, key moments, or a timestamped moments report in Match Lens style.
---

# Match Lens — Flagged Moments

Generates a flagged-moments report (match timeline + moments rail + timestamped detail) as a branded **Match Lens** PDF — a dark "ink" header band with the gold Match Lens identity, a category-coloured spine and accent edges, evidence-grade chips (A/B/C), and a match timeline and a key-moments rail above the timestamped detail.
...
2. **A `match_data.json`** describing the teams, events, shots and stat tiles (schema below). `report_type` must be `"flagged"`.
...
2. Build `match_data.json` to the schema below (set `report_type` to `"flagged"`).
3. Run:
   ```bash
...
```json
{
  "report_type": "flagged",            // fixed for this skill
