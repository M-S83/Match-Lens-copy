---
name: matchlens-pass-network
description: Create a branded Match Lens pass network report as a PDF: sequence summary, zone-flow and outcome tables, and pattern analysis in Match Lens style. Trigger when the user asks for a pass network, passing report, or sequence/zone analysis in Match Lens style.
---

# Match Lens — Pass Network

Generates a pass network report (sequence summary, zone-flow and outcome tables) as a branded **Match Lens** PDF — a dark "ink" header band with the gold Match Lens identity, a category-coloured spine and accent edges, evidence-grade chips (A/B/C), and styled sequence/zone tables and stat tiles.
...
2. **A `match_data.json`** describing the teams, events, shots and stat tiles (schema below). `report_type` must be `"pass_network"`.
...
2. Build `match_data.json` to the schema below (set `report_type` to `"pass_network"`).
3. Run:
   ```bash
...
```json
{
  "report_type": "pass_network",            // fixed for this skill
