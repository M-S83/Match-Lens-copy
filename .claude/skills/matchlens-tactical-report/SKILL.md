---
name: matchlens-tactical-report
description: Create a branded Match Lens tactical match report as a PDF: full-match breakdown with both teams' formation boards, a match timeline, a key-moments rail, headline stat tiles, and evidence-graded analysis. Trigger when the user asks for a tactical report, match analysis PDF, or full match breakdown in Match Lens style.
---

# Match Lens — Tactical Report

Generates a full tactical match report (both teams' formations, match timeline, key-moments rail, stat tiles) as a branded **Match Lens** PDF — a dark "ink" header band with the gold Match Lens identity, a category-coloured spine and accent edges, evidence-grade chips (A/B/C), and both teams' formation boards, a match timeline, headline stat tiles, and a key-moments rail.
...
2. **A `match_data.json`** describing the teams, events, shots and stat tiles (schema below). `report_type` must be `"tactical"`.
...
2. Build `match_data.json` to the schema below (set `report_type` to `"tactical"`).
3. Run:
   ```bash
...
```json
{
  "report_type": "tactical",            // fixed for this skill
